...
use valid component names from the schema, wrap JSON in ```json fences.''';
  ...
  static const multiTurnRefinement =
...
''',
      ...
      ..section(
        'Skill Library',
        '''When asked to browse skills or show capabilities:
...
- Use activate_skill / deactivate_skill actions for toggling''',
      ...
      ..section(
        'Control Tower',
        '''Additional management surfaces using the Vital Signs framework:
...
3. **Actions**:
   ...
   - Add a Button with action "save_itin" (context: city, days, vibe).''',
      ...
      ..section(
        ...
        '''You can embed inline vital sign charts using:
...
Optional: "dataPoints": [0.8, 0.9, 0.7, ...] (0.0-1.0), "width": 60, "height": 20.''',
  ...
  static PromptBuilder onboardingPrompt({required String a2uiSchema}) {
    return PromptBuilder()
      ...
      ..section(
        'Configuration Defaults (Pinned)',
        '''These values are PRE-SELECTED and shown to the user as a ready-to-launch proposal:
...
- Extras: --install-daemon, --non-interactive''',
  ...
  openclaw onboard --mode anthropic --auth key --install-daemon --non-interactive --json''')
      ..section(
        'Stage 1: Quick Start (Pre-configured)',
        '''Generate a "Quick Start" Card titled "System Ready: OpenClaw v2026.2.9":
...
- Voice: "Welcome to clawfree. I've prepared your OpenClaw environment with the latest stable build. Say 'Confirm' to launch immediately, or 'Custom' to change settings."''',
      ...
      ..section(
        'Stage 2: Pairing (after confirm_setup succeeds)',
        '''Generate a "Pair Devices" Card:
...
- On "Finished" or "I'm done" \u2192 trigger action "complete_onboarding".''',
      ...
      ..section(
        'Stage 3: Handover',
        '''When onboarding completes, tell the user:
...
- Do NOT generate new UI here \u2014 the system will switch to the Home prompt automatically.''',
      ...
      ..section(
        ...
        '''If the user says "Connect existing", "I have a gateway", "Link my gateway", or similar:
...
- On success the system will skip remaining onboarding and switch to Home mode with live health sync.''',
      ...
      ..section(
        'A2UI Rules',
        '''- Root id must be "root", createSurface first then updateComponents.
...
The user has completed setup. You are now in the Ready Home State.''',
      ...
      ..section(
        'Responsive Design',
        'Use the **ResponsiveContainer** to group cards or status indicators. This ensures the dashboard looks great on both iPad split-view and mobile screens.',
      )
      ..section(
        'Vital Signs Framework',
        '''Present system data using human-centric metaphors, NOT raw numbers:
...
- Never show raw ms, tokens, or percentages as primary labels. Use them as parenthetical detail only.''',
      ...
      ..section(
        'Home Dashboard \u2014 System Vitals',
        '''Generate a "System Vitals" Card showing:
...
- Key Monitors: Response Speed, Short-term Memory, Budget Remaining''',
      ...
      ..section(
        'Voice Commands \u2014 Agent Building',
        '''- "Create a new agent" / "Build an agent" \u2192 Switch to agent builder mode (action: "switch_to_builder").
- "List my agents" / "Show agents" \u2192 Display agent cards.''',
      )
      ..section(
        'Voice Commands \u2014 System Management',
        '''Respond to these with appropriate A2UI cards and execute the mapped OpenClaw commands:
...
- A Button with action "copy_pairing_link".''',
      ...
      ..section(
        'Voice Commands \u2014 Gateway Lifecycle (macOS Parity)',
        '''These map to the macOS native app's SSH gateway controls:
...
- Buttons for system_restart and system_upgrade''',
      ...
      ..section(
        'Voice Commands \u2014 System Tools (Native Parity)',
        '''These provide parity with OpenClaw's iOS/macOS native node capabilities:
...
Keep text concise (1-3 sentences) since they're read aloud.''')
      ..section(
        'A2UI Critical Rules',
        '''- EVERY ChoicePicker MUST include "value": [] \u2014 required to prevent rendering errors
- Root component id must be "root"
- Use createSurface first, then updateComponents to add content
